            ...
            <div className="flex-1 overflow-auto">
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody className={`divide-y ${isDarkMode ? 'divide-slate-800/30' : 'divide-slate-50'}`}>
                        ...
                        <DataItem label="Équipes Maçons" base="Par jour / éq." val={stats.teams * 0.4} isDarkMode={isDarkMode} />
                        <DataItem label="Équipes Réseau" base="Par jour / éq." val={stats.teams * 0.35} isDarkMode={isDarkMode} />
                        <DataItem label="Équipes Intérieur" base="Par jour / éq." val={stats.teams * 0.25} isDarkMode={isDarkMode} />

                        {/* 2. Logistics */}
                        <SectionHeader title="Logistique & Transport" total={stats.logistics} isDarkMode={isDarkMode} />
                        <DataItem label="Location Véhicules" base={`${duration} jours`} val={stats.logistics * 0.7} isDarkMode={isDarkMode} />
                        <DataItem label="Carburant (estimé)" base="Forfaitaire" val={stats.logistics * 0.3} isDarkMode={isDarkMode} />

                        {/* 3. Materials */}
                        <SectionHeader title="Matériaux (BOM)" total={stats.materials} isDarkMode={isDarkMode} />
                        <DataItem label="Kits Branchement (Type 1/2)" base={`${householdsCount} ménages`} val={stats.materials * 0.8} isDarkMode={isDarkMode} />
                        <DataItem label="Matériel Réseau" base="Poste forfaitaire" val={stats.materials * 0.2} isDarkMode={isDarkMode} />

                        {/* 4. Support */}
                        <SectionHeader title="Supervision & Support" total={stats.supervision} isDarkMode={isDarkMode} />
                        <DataItem label="Superviseurs Terrain" base="Ratio 1:10" val={stats.supervision * 0.6} isDarkMode={isDarkMode} />
                        <DataItem label="Chef de Projet" base="Mensuel" val={stats.supervision * 0.4} isDarkMode={isDarkMode} />
                    </tbody>
                </table>
            </div>
...
function DataItem({ label, base, val, isDarkMode }: { label: string, base: string, val: number, isDarkMode: boolean }) {
    return (
        <tr className={`group transition-colors ${isDarkMode ? 'hover:bg-slate-800/20' : 'hover:bg-indigo-50/30'}`}>
