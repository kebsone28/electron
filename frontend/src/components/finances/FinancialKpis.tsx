...
export default function FinancialKpis({ stats, devis }: { stats: any, devis: any }) {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 px-4">
            {items.map((item, i) => (
                <div key={i} className={`border rounded-[2rem] p-8 shadow-2xl relative overflow-hidden group transition-all ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100 hover:shadow-indigo-100/50'}`}>
                    <div className={`absolute top-0 right-0 p-8 opacity-5 text-indigo-500 group-hover:scale-110 transition-transform`}>
                        <item.icon size={100} />
                    </div>
            ...
            {/* Global Margin Card */}
            <div className={`md:col-span-2 lg:col-span-4 border rounded-[2.5rem] p-10 shadow-2xl relative overflow-hidden flex flex-col lg:flex-row items-center gap-10 transition-all ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`}>
                ...
                <div className="relative flex-1">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <h3 className={`text-xl font-black italic tracking-tighter uppercase ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Marge Prévisionnelle vs Devis</h3>
                    </div>
