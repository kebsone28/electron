...
export default function AgentsTab({ searchQuery = '' }: AgentsTabProps) {
    ...
    const activeAnomalies = filteredAgents?.flatMap(a => {
        const list = [];
        if (a.status === 'Inactif') {
            list.push({ 
                type: 'danger', 
    ...
    const getStatusColor = (status: string) => {
        switch(status) {
            case 'Actif': return isDarkMode ? 'bg-emerald-500/10 text-emerald-500' : 'bg-emerald-100 text-emerald-700';
            case 'Ralenti': return isDarkMode ? 'bg-amber-500/10 text-amber-500' : 'bg-amber-100 text-amber-700';
            default: return isDarkMode ? 'bg-rose-500/10 text-rose-500' : 'bg-rose-100 text-rose-700';
        }
    };
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className={`${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'} border rounded-2xl overflow-hidden transition-all hover:shadow-lg`}>
                    <div className={`p-6 border-b ${isDarkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'} flex justify-between items-center`}>
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className={`text-xl font-black ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Performances Agents</h3>
                                <p className={`text-xs font-medium mt-0.5 ${isDarkMode ? 'text-slate-500' : 'text-slate-600'}`}>Temps réel</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <table className={`w-full text-sm ${isDarkMode ? 'divide-slate-800' : 'divide-slate-200'}`}>
                            <thead className={isDarkMode ? 'bg-slate-950' : 'bg-slate-50'}>
                                <tr>
                                    ...
                                    <th className={`px-6 py-3 text-center text-xs font-semibold uppercase tracking-wider ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Tps Moyen</th>
                                    <th className={`px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Lastée</th>
                                    <th className={`px-6 py-3 text-right text-xs font-semibold uppercase tracking-wider ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Statut</th>
                                </tr>
                            </thead>
                            <tbody className={isDarkMode ? 'divide-y divide-slate-800' : 'divide-y divide-slate-200'}>
                                {filteredAgents.length > 0 ? filteredAgents.map((a, i) => (
                                    <tr key={i} className={isDarkMode ? 'hover:bg-slate-800/30' : 'hover:bg-slate-50'}>
                                        ...
                                        <td className={`px-6 py-4 text-center ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>
                                            <span className={`inline-flex items-center justify-center w-8 h-8 rounded-lg font-black ${isDarkMode ? 'bg-slate-800 text-primary' : 'bg-slate-100 text-primary'}`}>
                                                {a.visits}
                                            </span>
                                        </td>
                                        <td className={`px-6 py-4`}>
                                            <div className="flex flex-col items-center gap-2">
                                                <span className={`font-bold ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>{a.avgTime}m</span>
                                                <div className={`w-16 h-1.5 rounded-full overflow-hidden ${isDarkMode ? 'bg-slate-800' : 'bg-slate-200'}`}>
                                                    <div 
                                        ...
                                        <td className={`px-6 py-4 text-xs ${isDarkMode ? 'text-slate-500' : 'text-slate-600'}`}>
                                            {a.lastDate ? new Date(a.lastDate).toLocaleDateString('fr-FR') : '—'}
                                        </td>
                                        <td className={`px-6 py-4 text-right`}>
                                            <span className={`inline-flex items-center px-3 py-1.5 rounded-lg font-semibold text-xs ${getStatusColor(a.status)}`}>
                                                {a.status}
                                            </span>
                                        </td>
                                    </tr>
            ...
            <div className="space-y-6">
                <div className={`${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'} border rounded-2xl overflow-hidden transition-all hover:shadow-lg`}>
                    <div className={`p-6 border-b ${isDarkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'} relative`}>
                        <div className="absolute top-0 left-0 w-1 h-full bg-amber-500" />
                        <div className="flex items-center gap-3">
                            <div className="p-3 bg-amber-500/10 rounded-xl">
                                <AlertTriangle size={20} className="text-amber-500" />
                            </div>
                            <h3 className={`text-lg font-black ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Anomalies</h3>
                        </div>
                    </div>
                    <div className={`p-6 space-y-3 max-h-[500px] overflow-y-auto`}>
                        {activeAnomalies.length > 0 ? activeAnomalies.map((anom, i) => {
                            ...
                            return (
                                <div key={i} className={`${colors.bg} border ${colors.border} p-4 rounded-xl flex items-start gap-3`}>
                            ...
                            <div className={`flex flex-col items-center justify-center py-12 text-center ${isDarkMode ? 'text-slate-600' : 'text-slate-500'}`}>
                                <Zap size={32} className={`mb-3 ${isDarkMode ? 'text-slate-700' : 'text-slate-300'}`} />
