...
export default function DeliveriesTab({ searchQuery = '' }: DeliveriesTabProps) {
    ...
    const trackingList = households?.filter(h => (h.delivery?.date || h.koboSync) && h.id.toLowerCase().includes(searchQuery.toLowerCase())) || [];
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className={`${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'} border rounded-2xl p-6 transition-all hover:shadow-lg`}>
                    <div className="flex items-center justify-between mb-4">
                        <h3 className={`text-sm font-bold uppercase tracking-widest ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                            Kits Préparés (Kobo)
                        </h3>
            ...
            {/* Filters Bar */}
            <div className={`${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-slate-50 border-slate-200'} border rounded-2xl p-4 flex flex-wrap items-center justify-between gap-4 transition-all`}>
                <div className="flex items-center gap-3 flex-1 min-w-[280px]">
                    ...
                    <div className={`flex items-center gap-2 px-4 py-2.5 rounded-lg border ${isDarkMode ? 'bg-slate-800 border-slate-700 text-slate-300' : 'bg-white border-slate-300 text-slate-600'}`}>
                        <Calendar size={16} />
                        <span className="text-xs font-semibold whitespace-nowrap">Derniers 30j</span>
                    </div>
                </div>
                <button className={`flex items-center gap-2 px-4 py-2.5 rounded-lg font-semibold transition-all ${isDarkMode ? 'bg-slate-800 border border-slate-700 text-slate-300 hover:bg-slate-700' : 'bg-white border border-slate-300 text-slate-600 hover:bg-slate-50'}`}>
                    <Filter size={16} />
                    ...
                    <table className={`w-full text-sm ${isDarkMode ? 'divide-slate-800' : 'divide-slate-200'}`}>
                        <thead className={isDarkMode ? 'bg-slate-950' : 'bg-slate-50'}>
                            <tr>
                                ...
                                <th className={`px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Phases Déploiement</th>
                                <th className={`px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Matériaux</th>
                                    ...
                                    <tr key={i} className={isDarkMode ? 'hover:bg-slate-800/30' : 'hover:bg-slate-50'}>
                                        ...
                                        <td className={`px-6 py-4 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>
                                            <div className="flex items-center gap-3">
                                                <div className={`w-8 h-8 rounded-full flex items-center justify-center border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-slate-100 border-slate-300'}`}>
                                        ...
                                        <td className={`px-6 py-4 text-right`}>
                                            <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg font-semibold text-xs ${finalOk ? (isDarkMode ? 'bg-emerald-500/10 text-emerald-500' : 'bg-emerald-100 text-emerald-700') : (isDarkMode ? 'bg-amber-500/10 text-amber-500' : 'bg-amber-100 text-amber-700')}`}>
                                    ...
                                    <td colSpan={5} className={`px-6 py-20 text-center`}>
                                        <div className="flex flex-col items-center justify-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className={`text-xs ${isDarkMode ? 'text-slate-600' : 'text-slate-500'}`}>Aucun ménage avec livraison trouvé</p>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
