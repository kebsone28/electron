...
export const MapRouting: React.FC<MapRoutingProps> = ({ enabled, onClose, startPoint, endPoint }) => {
    const map = useMap();
    ...
    const fetchRoute = async () => {
        ...
        try {
            const resp = await fetch(`https://router.project-osrm.org/route/v1/${osrmMode}/${coords}?overview=full&geometries=geojson&steps=true`);
            const data = await resp.json();
            if (data.routes && data.routes.length > 0) {
                setRoute(data.routes[0]);
                // Fit bounds if needed
                const polyline = L.polyline(data.routes[0].geometry.coordinates.map((c: any) => [c[1], c[0]]));
