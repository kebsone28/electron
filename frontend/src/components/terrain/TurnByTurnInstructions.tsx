...
import { ChevronDown, ChevronUp, Navigation, Clock, Gauge, Volume2, VolumeX, AlertCircle } from 'lucide-react';
...
export const TurnByTurnInstructions: React.FC<TurnByTurnInstructionsProps> = ({
    ...
    const distanceToDestination = instructions.slice(currentStep).reduce((sum, step) => sum + step.distance, 0);
    ...
    const getManeuverIcon = (instruction: string | undefined): string => {
        ...
        if (inst.includes('continuer') || inst.includes('continue') || inst.includes('straight')) return '⬆️';
        ...
        >
            ...
            <AnimatePresence>
                {expanded && (
                    ...
                    >
                        {/* Progress Bar */}
                        <div className={`px-5 py-3 border-b ${isDarkMode ? 'border-slate-700 bg-slate-800/50' : 'border-slate-100 bg-slate-50'}`}>
                            <div className="flex items-center justify-between text-xs font-black uppercase tracking-widest mb-2 text-slate-500">
                                <span>Progression</span>
                                <span>Étape {currentStep + 1} / {instructions.length}</span>
                            </div>
                        ...
                        <div className={`px-5 py-4 border-b ${isDarkMode ? 'border-slate-700 bg-blue-500/10' : 'border-slate-100 bg-blue-50'}`}>
                            <div className="flex items-start gap-4">
                                <div className="text-3xl flex-shrink-0">{getManeuverIcon(instructions[currentStep]?.instruction)}</div>
                                <div className="flex-1 min-w-0">
                                    <p className="font-black text-sm uppercase tracking-widest text-blue-600 mb-1">Étape actuelle</p>
                                    ...
                                    <div className="flex gap-4 mt-3">
                                        <div className="flex items-center gap-1.5">
                                            ...
                                            <span className="text-sm font-bold">{formatDistance(instructions[currentStep]?.distance || 0)}</span>
                                        </div>
                                        <div className="flex items-center gap-1.5">
                                            <Clock size={14} className="text-emerald-500" />
                                            <span className="text-sm font-bold">{formatDuration(instructions[currentStep]?.duration || 0)}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        {/* Steps List */}
                        <div className={`max-h-96 overflow-y-auto ${isDarkMode ? 'divide-slate-700' : 'divide-slate-100'} divide-y`}>
                            {instructions.map((step, index) => (
                                ...
                                >
                                    <div className="flex items-center gap-3 mb-1">
                                        <span className="text-2xl flex-shrink-0">{getManeuverIcon(step.instruction)}</span>
                                        <div className="flex-1 min-w-0">
                                            <p className="font-bold text-sm truncate">{step.instruction || 'Continue'}</p>
                                            {step.name && (
                                                <p className={`text-xs truncate ${isDarkMode ? 'text-slate-500' : 'text-slate-600'}`}>
                                                    {step.name}
                                                </p>
                                            )}
                                        </div>
                        ...
                        {/* Navigation Controls */}
                        <div className={`flex gap-2 p-4 border-t ${isDarkMode ? 'border-slate-700 bg-slate-800/50' : 'border-slate-100 bg-slate-50'}`}>
                            <button
