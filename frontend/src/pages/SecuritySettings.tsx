...
function PasswordField({ value, onChange, placeholder, title: fieldTitle }: {
...
export default function SecuritySettings() {
    ...
    const changeProjectDeletePassword = async () => {
        ...
        setProjDelCur(''); setProjDelNew(''); setProjDelConf('');
    ...
    return (
        <div className="min-h-screen bg-slate-950 p-4 md:p-8">
            ...
            <div className="fixed top-6 right-6 z-[9999] flex flex-col gap-2 pointer-events-none">
                {toasts.map(t => (
                    <div key={t.id} className={`flex items-center gap-3 px-5 py-4 rounded-2xl shadow-2xl font-bold text-sm pointer-events-auto ${t.type === 'success' ? 'bg-emerald-600 text-white' :
            ...
            <div className="max-w-3xl mx-auto space-y-6">
                ...
                <Section icon={Lock} title="Mot de passe — Suppression de projet" color="rose"
                    ...
                    <form onSubmit={(e) => { e.preventDefault(); changeProjectDeletePassword(); }} className="space-y-3">
                        <div className="sr-only" aria-hidden="true">
                            <input type="text" name="username" value="admin" readOnly autoComplete="username" tabIndex={-1} />
                        </div>
                        <div>
                ...
                <Section icon={ShieldCheck} title="Mot de passe administrateur" color="indigo"
                    subtitle="Utilisé pour les actions sensibles (suppression compte admin, accès paramètres).">
                    <form onSubmit={(e) => { e.preventDefault(); changeAdminPassword(); }} className="space-y-3">
                        <div className="sr-only" aria-hidden="true">
                            <input type="text" name="username" value="admin" readOnly autoComplete="username" tabIndex={-1} />
                        </div>
                        <div>
                ...
                <Section icon={HelpCircle} title="Question de sécurité" color="amber"
                    subtitle="Utilisée pour récupérer l'accès si le mot de passe admin est oublié.">
                    <form onSubmit={(e) => { e.preventDefault(); changeSecurityQuestion(); }} className="space-y-3">
                        <div className="sr-only" aria-hidden="true">
                            <input type="text" name="username" value="admin" readOnly autoComplete="username" tabIndex={-1} />
                        </div>
                        <div>
                ...
                <Section icon={QrCode} title="Code de récupération d'urgence" color="emerald"
                    ...
                    <div className="space-y-4">
                        {recoveryCode ? (
                            <div className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-between gap-4">
                                <span className="font-mono font-bold text-emerald-300 tracking-widest text-sm">{recoveryCode}</span>
                                <button onClick={copyCode} title={copied ? 'Copié !' : 'Copier'}
                        ...
                        <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/20">
                            <p className="text-amber-300 text-xs font-bold flex items-start gap-2">
                                ...
                                Conservez ce code dans un endroit sûr (imprimé ou dans un gestionnaire de mots de passe). Il sera invalidé après utilisation.
                            </p>
                        </div>
                ...
                <div className="border border-dashed border-slate-700 rounded-3xl p-6 flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-slate-500 text-sm">Récupérez l'accès via la question de sécurité ou le code d'urgence.</p>
                        </div>
                    </div>
