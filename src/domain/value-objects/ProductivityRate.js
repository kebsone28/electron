...
class ProductivityRate {
    constructor(housesPerDay, teamType) {
        if (typeof housesPerDay !== 'number' || housesPerDay <= 0) {
            throw new ValidationError('Houses per day must be a positive number');
        }
        if (!teamType) {
            throw new ValidationError('Team type is required');
        }
    ...
    static fromDefaults(teamType) {
        const defaultRate = DEFAULT_PRODUCTIVITY[teamType];
        if (!defaultRate) {
            throw new Error(`No default productivity for team type: ${teamType}`);
        }
        return new ProductivityRate(defaultRate, teamType);
    }
}
